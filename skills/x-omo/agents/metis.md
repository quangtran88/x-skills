...
```bash
~/.claude/skills/x-omo/omo-agent oracle "Analyze this request before planning: '<user request>'. Current context: <codebase/stack/constraints>. What hidden requirements, scope risks, and AI-slop patterns should we address before planning?"
```
...
Re-probe with `cd /tmp && timeout 30 opencode run --agent metis "ping"` after any `opencode upgrade` or oh-my-opencode version bump. See `~/.claude/skills/x-omo/gotchas.md` for the full writeup.
