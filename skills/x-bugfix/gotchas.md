...
## basic-memory: project targeting & silent wrong-store writes

basic-memory is single-process (markdown files + SQLite index, MCP over stdio — verified at `research/basicmachines-co/basic-memory/src/basic_memory/cli/commands/mcp.py:28`, default transport `stdio`). There is no separate backend server, no standalone-vs-proxy split, no health probe. If a `mcp__basic-memory__*` call errors despite `mcp.basic_memory` being pinned, that is ordinary capability drift (server deregistered, `uvx` missing) — handle per `../x-shared/capability-loading.md § Drift Handling`; do not retry in a loop.

The real trap is **project targeting**: every tool takes an optional `project` and resolves a default when omitted. With multiple knowledge bases configured, a `write_note` without an explicit `project` succeeds silently into whatever the default resolves to — and the matching recall later comes back empty. When a `search_notes` that should hit returns nothing, check `list_memory_projects` before concluding the lesson was never saved. Placement + tagging conventions are canonical in `../x-shared/mcp-toolbox.md § basic-memory`.

There is also no session-replay/commit-linkage tier (the old agentmemory `file-context` / `by-commit` routes have no basic-memory equivalent). For regression hunts over file history, go straight to `git log -p -- <file>`.
