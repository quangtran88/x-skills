...
- [ ] **Persist lesson** (only when `mcp.basic_memory` pinned): one `mcp__basic-memory__write_note({ title: "<area>: <what was built/changed>", directory: "lessons/<project-slug>", content: "<one-line: what was built/changed> → <observed outcome>\n\nFiles: <touched paths>", tags: ["<project-slug>", "x-do", "<mode A/B/C/D>", "<area>"] })` call (project-slug = basename of cwd — see `../../x-shared/mcp-toolbox.md § Consumer rules`). Skip silently when not pinned.
