...
6.6. **Persist lesson** — § Memory Reflex persist beat (`../../x-shared/mcp-toolbox.md`), gated on `mcp.basic_memory` pinned. Runs HERE — before branch finish — deliberately: `finishing-a-development-branch` may merge, switch back to main, or remove the worktree, making cwd (and any cwd-derived project-slug) non-deterministic afterward. Apply the **durability gate first**: only write when the run surfaced a genuine gotcha, root cause, or non-obvious lesson worth a future session — a routine successful build ("built X, tests pass") is NOT durable, so **skip the write** on those. When a real lesson passed the gate: `mcp__basic-memory__write_note({ title: "<area>: <the lesson>", directory: "lessons/<project-slug>", content: "<the gotcha / root cause / non-obvious finding + why it matters>", tags: ["<project-slug>", "x-do", "<area>"] })` (project-slug per § Consumer rules — worktree-safe resolution). If this run's Pre-Flight recall already surfaced a note on the same lesson **and the same kind**, update it instead of writing a duplicate — use the *Update over duplicate* shape in § Memory Reflex (`edit_note` with the recall hit's **permalink** as `identifier` and `operation: "append"`; it cannot retag or move a note, so write fresh if the surfaced note is misfiled — and NEVER append across kinds, e.g. onto a `decisions/` note x-backlog filed for the same slug). When the run came through the backlog chain, cite the earlier note's permalink instead of restating it (§ Memory Reflex "Chained skills link, don't restate"). Skip silently when not pinned.
...
The § Memory Reflex persist beat already ran at Execution step 6.6 (before branch finish) — do not re-run it here.
