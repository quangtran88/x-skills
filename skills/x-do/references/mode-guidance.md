...
1. Read the plan fully
   - **Backlog doc?** Path under `docs/backlog/` → apply `../SKILL.md` § "Backlog Doc Lifecycle
     (Mode A)": worktree suggestion + `status: in-progress` flip now, archival on completion
     (`steps/step-04-execute.md` § Execution step 6.5, after commit recomposition).
