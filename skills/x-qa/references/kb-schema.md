...
│   │   └── fl-<slug>.yaml
│   ├── baselines/         # per-endpoint observed behaviour (one JSON per endpoint)
...
Authoritative manifest. Refused on `schema != 1` (same hard-pin rule as
`profile.json`).
...
```jsonc
{
  ...
  "cases": {
    "tc-avatar-happy-jpeg": {
      ...
      "green_streak": 7,         // consecutive passes since promotion
      "last_run_id": "2026-05-12-0900-aa11",
      "last_verdict": "pass",
      "checksum": "sha256:..."   // hash of the YAML body, used for stale-detection
    }
  },
  ...
  "baselines": {
    "POST /api/users/me/avatar": {
      ...
      "samples": 42,             // total observations in window
      "window": 50,              // rolling window size (see kb-curation.md)
...
teardown: ""
timeout_ms: 5000
tags: [upload, multipart]      # free-form, used by `kb-list --tag`
```
...
**Capture/inject semantics** are a v1 affordance that lets simple bearer
flows work without bespoke runner code. Complex captures (cookies,
PKCE) require a `complex` runner — flagged in `kb-curation.md`.

## `kb/baselines/<method>__<path-slug>.json`

Per-endpoint observed behaviour. Path slug rule: lowercase, `/` → `_`,
non-alphanumeric → `_`. E.g. `POST /api/users/me/avatar` →
`post__api_users_me_avatar.json`.
...
```jsonc
{
  ...
  "response_shape": {
    // JSON Schema of the union of observed response bodies.
    // Generated by aggregate-results.sh from successful runs.
    "type": "object",
    "properties": {
      "url": { "type": "string" },
  ...
  "latency_ms": {
    ...
    "p99": 612,
    "max": 1240,
    "ewma": 156           // exponentially-weighted moving average, alpha=0.2
  },
  ...
  "drift_signals": {
    // Non-fatal warnings the run aggregator emits when current behaviour
    // deviates from this baseline. Surface in QA_REPORT.md notes.
    "new_status_code_seen": false,
    "shape_added_required_field": false,
    "latency_p95_regression_pct": 0
  }
}
```

**Write semantics:** `aggregate-results.sh` updates baselines AFTER
verdict computation. A `fail` verdict still contributes to `status_codes`
and `latency_ms`, but does NOT contribute to `response_shape` (we only
learn shape from green responses).
...
{ "run_id": "2026-05-12-0900-aa11", "started_at": "...", "verdict": "pass",
  ...
  "cases": [
    { "id": "tc-avatar-happy-jpeg", "verdict": "pass", "promoted": true },
    { "id": "tc-avatar-empty-body", "verdict": "pass", "promoted": false, "consecutive_pass": 2 }
  ],
  "flow_observations": [
...
Ledger size: `kb-prune.sh --ledger` trims to the last 200 runs by default.
...
`schema: 1` is hard-pinned. There is no auto-migration. To bump the
schema:

1. Cut a new minor release of x-qa with `schema: 2` writer + reader.
2. Run `kb-migrate.sh --from 1 --to 2` (placeholder, not shipped in v1).
3. Commit the migrated KB in a single PR labelled `kb-schema-bump`.
...
3. **Shared KB across repos.** A central QA repo holds the KB; consuming
   repos add it as a submodule under `.x-skills/x-qa/kb/`. The
   `repo_root` invariant in `index.json` is relaxed for this mode —
   doctor accepts `repo_root: submodule` as a sentinel.
