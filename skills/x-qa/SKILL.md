...
1. Bootstrap (above).
   - [ ] **Memory recall** (only when `mcp.basic_memory` pinned in bootstrap-active set): one `mcp__basic-memory__search_notes({ query: "<test path or framework + 'flake'>", page_size: 10 })` call. Surface prior flake notes as test-history context for case classification — not autopilot. **Apply consumer rules from `../x-shared/mcp-toolbox.md § Consumer rules`.** When `mcp.basic_memory` is not pinned, **skip silently** — Claude's native auto-memory file still applies.
